...
extension DebugViewController: UITableViewDelegate, UITableViewDataSource, UIAlertViewDelegate {
	...
	func tableView(_ tbaleView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
	...
	func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
		...
		case .testLeanCloud:
			...
			myntlog.saveInBackground({ (succeed, error) in
				MTToast.show(succeed ? "successed": "failed \(error)")
			})
		case .webserver:
			if WebServer.shared.isOpen {
				Toast.show(message: "webserver已关闭,如果使用过程中出现Crash,请重启app")
				WebServer.shared.close()
			} else {
				Toast.show(message: "webserver已启动，如果使用中出现Crash，请重启app")
	...
	func alertView(_ alertView: UIAlertView, clickedButtonAt buttonIndex: Int) {
		...
		case DebugType.superdebug.rawValue:
			if buttonIndex == 1 {
				...
				if let email = MYNTKit.shared.user?.email.lowercased() {
