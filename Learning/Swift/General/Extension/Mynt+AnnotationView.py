...
extension UIView {
	...
	func showNavigationView(mapMynt: MapMynt, clickHandler: @escaping (MapMynt) -> Void) {
		...
		let view = NavigationView(frame: CGRect(x: bounds.width / 2 - width / 2, y: -height - 5, width: width, height:
			height))
		view.clickHandler = {
	...
	func showMapMyntCollectionView(mapMynts: [MapMynt]) {
		...
		let view = showMapMyntCollectionView(frame: CGRect(x: bounds.width / 2 - width / 2, y: height / 2, width: width, height:
			height))
...
extension Mynt {
	...
	func annotationImage(count: Int = 1, showOffline: Bool = true, block: @escaping (UIImage?) -> Void) {
		let width = Mynt.annotationWigth
		...
		loadAvator { image in
			...
			DispatchQueue.global().async {
				...
				//绘制外圈圆
				var path = UIBezierPath(arcCenter: CGPoint(x: width / 2, y: width / 2), radius: width / 2, startAngle: 0, endAngle: 180, clockwise: true)
				...
				if showOfflineLayer {
					//绘制蒙版
					path = UIBezierPath(arcCenter: CGPoint(x: width / 2, y: width / 2), radius: width / 2 - 2, startAngle: 0, endAngle: 180, clockwise: true)
					context?.addPath(path.cdPath)
					context?.setFillColor(UIColor(white: 1, alpha: 0.8).cgColor)
					context?.drawPath(using: .fill)
				}
				...
				if count > 1 {
					...
					var center = CGPoint(x: width - radius, y: radius)
					path = UIBezierPath(arcCenter: center, radius: radius, startAngle: 0, endAngle: 180, clockwise: true)
					context?.addPath(path.cgPath)
					context?.setFillColor(UIColor(red:0.40, green:0.60, blue:0.99, alpha:1.00).cgColor)
					context?.drawPath(using: .fill)

					let font = UIFont.systemFont(ofSize: 12)
					let text = String(format: "%d", conut)
