...
def printProfile(skypeDB):
	...
	c = conn.cursor
	c.excute("SELECT fullname, skypename, city, country, \
...
def printContacts(skypeDB):
	...
	conn = sqlite3.connect(skypeDB)
	c = conn.cursor()
	c.excute("SELECT displayname, skypename, city, country,\
...
def printCallLog(skypeDB):
	...
	c.execute("SELECT datetime(begin_timestamp,'unixepoch'),\
		identify FROM calls, conversations WHERE \
