...
class FitModel: NSObject {
    ...
    ///参数：当前配速（单位：米/秒）
    ///返回值：字典  类型：字典
    class func dictionaryFromTimePace(pace: Float) -> Dictionary<String, String>{
        if pace == 0{
            return ["min":"-", "sec":"-"]
        }else{
    ...
    ///参数：当前用时（单位：毫秒）
    ///返回值：字典  类型：字典
    class func dictionaryFromTimeInterval(interval: Double) -> Dictionary<String, Int> {
    ...
    ///参数：当前运动状态（类型：字符串）
    ///返回值：无  类型：无
    class func reportStatus(status: String){
        var array:[String] = []
        switch status {
        case "start":
    ...
    ///参数2：所用时间（单位：秒）
    ///返回值：无  类型：无
    class func reportDistance(distance: Int, time: TimeInterval){
        ...
        if distance < 2{
            ...
            DispatchQueue.once(token: String(distance)) {
                ...
                recentTimeArray.append(time)
                switch time {
                case 0..<60:
                    array = ["Rprompt", "Rrunned_distance", "N000", "Rkm", "Rtimecost", "N000", "Rseconds"]
                    //获取秒数音轨
                    let secondsStr = getTrackName(number: t["s"]!)
                    if secondsStr != ""{
                        array[5] = secondsStr
                    }
                    break
                case 60..<3600:
                    ...
                    //获取分钟数音轨
                    let minutesStr = getTrackName(number: t["m"]!)
                    if minutesStr != ""{
                        array[5] = minutesStr
                    }
                    //获取秒数音轨
                ...
                default:
                    ...
                    //获取小时音轨
                    let hoursStr = getTrackName(number: t["h"]!)
                    if hoursStr != ""{
                        array[5] = hoursStr
                    }
                    //获取分钟数音轨
                ...
                //获取公里数音轨
                let distanceStr = getTrackName(number: distance)
            ...
            DispatchQueue.once(token: String(distance)) {
                ...
                var array:[String] = []
                recentTimeArray.append(time)
                let recent = recentTimeArray[recentTimeArray.count - 2]
                let t = dictionaryFromTimeInterval(interval: time)
                ...
                case 0..<60:
                    ...
                    //获取前1公里秒数音轨
                    let recentSecondsStr = getTrackName(number: r["s"]!)
                    if recentSecondsStr != ""{
                ...
                case 60..<3600:
                    ...
                    //获取前1公里分钟数音轨
                    let recentMinutesStr = getTrackName(number: r["m"]!)
                    if recentMinutesStr != ""{
                ...
                default:
                    ...
                    //获取前1公里小时音轨
                    let recentHoursStr = getTrackName(number: r["h"]!)
                    if recentHoursStr != ""{
