...
class JFTipView: UIView {
	...
	*/
	func show() {
		...
		bgView.addGestureRecognizer(UITapGestureRecognizer(target: self, action: #selector
			(didTappedBgView(_:))))

		UIApplication.shared.keyWindow?.addSubview(bgView)
		UIApplication.shared.keyWindow?.addSubview(self)

		swipeImageView.image = UIIamge(named: "swipe_down")
		...
		swipeLabel.frame = CGRect(x: 160, y: SCREEN_HEIGHT - 180, width: SCREEN_WIDTH - 160,
			height: 30)
	...
	*/
	fileprivate func showAnimation(_ count: Int) {
		...
		UIView.animate(withDuration: 0.3, delay: 0, options: UIViewAnimationOptions.
			allowUserInteraction, animtions: {
	...
	*/
	func dismiss () {
		UIView.animate(withDuration: 0.25, animation: {
