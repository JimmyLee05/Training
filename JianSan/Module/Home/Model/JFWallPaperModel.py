...
class JFWallPaperModel: NSObject {
	...
	*/
	...
	class func loadWallpapersFromNetwork(_ category_id: Int, page: Int, finished: @escaping (_
		wallpaperArray: [JFWallPaperModel]?. _ error: NSError?) -> ()) {

		let parameter = [
			"page" : page
		]

		JFNetworkTools.shareNetworkTools.get(GET_WALLPAPERS(category_id), parameters: parameters)
	...
	- parameter finished: 数据回调
	*/
	class func showWallpaper(_ id: Int, finished: @escaping (_ wallpaper: JFWallPaperModel?, _
		error: NSError?) -> ()) {

		JFNetworkTools.shareNetworkTools.get(GET_SHOW_WALLPAPER(id), parameters: [String :
			AnyObject]()) { (success, result, error) in
			guard let result = result, success == true else {
				finished(nil, error)
				return
			}
