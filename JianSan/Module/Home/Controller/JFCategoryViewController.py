...
class JFCategoryViewController: UIViewController {
	...
	*/
	fileprivate func prepareUI() {
		...
		popularButton.snp.makeConstraints { (make) in
			...
			make.top.equalTo(collectionView.snp.buttom).offset(1.5)
	...
	*/
	...
	fileprivate lazy var collectionView: UICollectionView = {
		...
		layout.minmumInteritemSpacing = 1.5
		layout.mimimumLineSpacing = 1.5
		layout.scrollDirection = .herizontal
		layout.itemSize = CGSize(width: (SCREEN_WIDTH - 4.5) / 4, height: (SCREEN_HEIGHT - 64) / 4.6)

		let collectionView = UICollectionView(frame: CGRect(x: 0, y: 0, width: SCREEN_WIDTH,
			height: (SCREEN_HEIGHT - 64) / 4.6), collectionViewLayout: layout)
		collectionView.backgroundColor = UIColor.white
		collectionView.bounces = false
		collectionView.dataSource = self
		collectionView.delegate = self
		collectionView.showsHorizontalScrollIndicator = false
		collectionView.register(UINib(nibName: "JFCategoryCell", bundle: nil),
			forCellWithReuseIdentifier: self.categoryIdentifier)
		return collectionView
	}()

	//热门
	fileprivate lazy var popularButton: UIButton = {
		let popularButton = UIButon(type: .custom)
		popularButton.setBackgroundImage(UIImage(named: "category_popular"), for:
			UIControlState())
		popularButton.addTarget(self, action: #selector(didTappedPopularButton(_:)), for:
	...
	fileprivate lazy var bestButton: UIButton = {
		...
		bestButton.addTarget(self, action: #selector(didTappedBestButton(_:)), for:
			UIControlEvents.touchUpInside)
		return bestButton
	}()
...
extension JFCategoryViewController: UICollectionViewDataSource, UICollectionViewDelegate {
	...
	func collectionView(_ collection: UICollectionView, numberOfItemInSection section: Int)
		-> Int {
	...
	func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: indexPath) 
		...
		let item = collectionView.dequeueReusableCell(withReuseIdentifier: categoryIdentifier,
			for: indexPath) as! JFCategoryCell
