...
class JFCollectionViewController: UIViewController {
	...
	//当前角标
	 ...
	 //卡片视图
	 var swipeableView = ZLSwipeableView(frame: CGRect(x: 50, y: 10, width: SCREEN_WIDTH - 100,
	 ...
	 */
	 fileprivate func prepareUI() {
	 	...
	 	swipeableView.didTap = {view, location in
	 		...
	 		//临时放大动画的图片}
	 		let tempView = UIImageView(image: YYImageCache.shared().getImageForKey("\(BASE_URL)/\
	 			(self.data[view.tag]["path"]!)"))
	 		UIApplication.shared.keyWindow?.insertSubview(tempView, aboveSubview: view)

	 		tempView.frame = CGRect(x: 50, y: 74, width: SCREEN_WIDTH - 100, height:
	 			SCREEN_HEIGHT - 74)
	...
	@objc fileprivate func loadData() {
		...
		JFFMDBManager.sharedManager.getStarWallpaper { (result) in
			...
			for (index, dict) in result.enumerated() {
				let imageView = UIImageView(image: YYImageCache.shared().getImageForKey("\
					(BASE_URL)/\(dict["path"]!)"))
