...
- (void)KVOHandler {
	...
	[self.KVOController observer:self.viewModel keyPath:@"netFail" options:NSKeyValueObservingOptionNew block:^(id observer, id object, NSDictionary *change) {
	...
	[self.KVOController observer:self.viewModel keyPath:@"registerSuccessOrFail" options:NSKeyValueObservingOptionNew block:^(id observer, id object, NSDictionary *change) {
		if ([self.viewModel.registerSuccessOrFail boolValue]) {
			[_HUD hideAnimated:YES];
			[[[self presentingViewController]presentingViewController] dismissViewControllerAmimated:YES completion:nil];
		}else {
			[_HUD hideAnimated:YES];
			[Utils showTextHUDWithText:self.viewModel.invalidMsg addToView:self.view];
		}
...
- (IBAction)closeBtnClick:(UIButton *)sender {
	[self dismissViewControllerAmimated:YES completion:nil];
}
