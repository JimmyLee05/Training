...
public class CalendarView: UIView, UICollectionViewDataSource, UICollectionViewDelegate {
	...
	fileprivate func commitIn() {
		...
		for i in (0..<4).reversed() {
			let timeInterval = Date().timeIntervalSince1970 - Doubel(i) * 86400 * 7
			let date = Date(timeIntervalSince1970: timeInterval).firstDayInWeek()
			for i in 0..<7 {
				let date = Date(timeIntervalSince1970: date.timeIntervalSince1970 + Double(i) * 86400)
				dates.append(date)
        ...
        collectionView?.register(CalendarCollectionViewCell.self, forCellWithReuseIdentifier: 
        	"CalendarCollectionViewCell")
        addSubview(collectionView!)
        updateCollectionViewConstraints()1
	}
	...
	public override func layoutSubviews() {
		...
		collectionView?.setContentOffset(CGPoint(x: collectionView.contentSize.width - collectionView.frame.width,
			y: 0), animated: false)
	}
	...
	fileprivate func updateCollectionViewConstraints() {
		...
		widthConstraints = NSLayoutConstraint.constraints(widthVisualFormat: "H:|-(d)-[view]-(d)-|",
														  options: .directionLeftToRight,
														  metrics: ["d": bounds.height / 2],
														  views: ["view": collectionView!])
		heightConstraints = NSLayoutConstraint.constraints(widthVisualFormat: "V:|-0-[view]-0-|",
														   options: .directionLeftToRight,
														   metrics: ["d": bounds.height / 2],
														   views: ["view": collectionView!])
	...
	public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) ->
		...
		let cell = collectionView.dequeueReusableCell(forCellWithReuseIdentifier: "CalendarCollectionViewCell", for:
			indexPath) as? CalendarCollectionViewCell
		let date = dates[indexPath.row]
		cell?.label?.text = "\(date.dayString)"
		cell?.label?.backgroundColor = selectedDate == date ? UIColor(red:0.24, green:0.56, blue:1.00, alpha:
			1.00) : .clear
		cell?.label?.textColor = selectedDate == date ?
			.white :
			date.timeIntervalSince1970 > todayDate.timeIntervalSince1970 ?
		...
		public func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
			...
			dalegate?.didSelectDay(calendarView: self, date: date)
		}
}
