//
//  NewMyntInfoExpandSubView.swift
//  MYNT
//
...
class MyntInfoControlExpandSubView: MyntInfoBaseSubView {
    ...
    class ClickEventView: UIView {
        ...
        fileprivate lazy var borderlayer: CALayer = {
            ...
            layer.borderWidth   = 1
            layer.borderWidth   = UIColor(red:0.91, green:0.91, blue:0.91, alpha:1.00).cgColor
    ...
    lazy var tripleClickView: ClickEventView = {
        let view = ClickEventView()
    ...
    lazy var holdView: ClivkEventView = {
        let view = ClickEventView()
    ...
    override func updateUIData(mynt: Mynt) {
        ...
        doubleClickView.valueLabel.text = mynt.currentControl.doubleClick.name
    ...
    @objc fileprivate func didClickValueView(gestrueRecognizer: UIGestureRecognizer) {
        if let view = gestureRecognizer.view as? ClickEventView {
            viewController?.didSelectControllerValue(control: items[coverflowView.selectedIndex], event: view.clickEvent)
        }
    }
}
...
extension MyntInfoControlExpandSubView: MTCoverFlowViewDelegate {
    ...
    func numberOfItemsInCoverFlowViewm(_ collectionView: MTCoverFlowView) -> Int {
        return items.count
    }
