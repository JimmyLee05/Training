...
typelias DialogSelectionHandler = (_ dialog: BaseDialog?, _ type: SelectionDialogView.SelectedType) -> Void
...
class SelectionDialogView: DialogBaseView {
	...
	var selectedType: SelectedType = .left {
		didSet {
			...
			leftView.backgroundColor = selectedLeft ? UIColor(red:0.91, green:0.91, blue:0.91, alpha:1.00) :
				UIColor.clear
			rightView.backgroundColor = !selectedLeft ? UIColor(red:0.91, green:0.91, blue:0.91, alpha:1.00) :
				UIColor.clear
	...
	override func awakeFromNib() {
		...
		leftView.addGestrueRecognizer(UITapGestureRecognizer(target: self, action: #selector(didClickBuyTypeView
			(tapGestureRecognizer:))))
		rightView.addGestrueRecognizer(UITapGestureRecognizer(target: self, actionL: #selector(didClickBuyTypeView
			(tapGestureRecognizer:))))
	}
