...
class DialogManager: NSObject {
	...
	fileprivate var dialogQueue = [BasicDialog]()

	var currentDialog: BaseDialog?

	var limitViewController = [String]()
	...
	public func register(types: [String]) {
		limitViewController = types
	}

	func removeFromQueue(type: DialogManager.DialogType) {
		if let result  = dialogQueue.first(where: { $0.dialog == type }) {
			dialogQueue.remove(object: result)
		}
	}

	func addToQueue(dialog: BaseDialog) {
		if dialogQueue.first(where: { $0.dialogType == dialog.dialogType }) == nil {
	...
	- parameter rigntName 					右侧选择名字
	- parameter buttonString 				按键名字
	...
	*/
	...
	public func showSelectionDialog(dialogType: DialogManager.DialogType,
		...
		view?.rightImageView.image 		= rightImage
	...
	public func showShareDialog(clickLinkHandler: @escaping DialogClickHandler,
								cilciWechatHandler: @escaping DialogClickHandler) {
		let dialogType = DialogManager.DialogType.askHelp

		let view = AskHelpDialogView.createFormXib() as? AskHelpDialogView
		view?.dialogClickHandler 		= clickLinkHandler
	...
	public func show(title: String,
		...
		view?.okButton.addTarget(dialog!, action: #selector(BaseDialog.didClickOkButton(button:)), for: .
			touchUpInside)
	...
	public func showDatePickers(title: String,
		...
		dialog?.titleLabel.text 			= title
		dialog?.titleLebal.numberOfLines 	= 0
		dialog?.dialogClickOkHandler 		= clickOkHandler
		view?.okButton.addTarget(dialog!, action: #selector(BaseDialog.didClickOkButton(button:)), for: .
			touchUpInside)
		dialog?.show()
	...
	public func show(permissionType: STPermissionType,
		...
		let dialogType = DialogManeger.DialogType.permission
		...
		dialog?.dialogType 				= dialogType
		dialog?.imageView.image 		= dialogType.image
		dialog?.titieLabel.text 		= permissionType.dialogTitle
		fialog?.dialogCLickOkHandler 	= clickOkHandler
		view?.okButton.addTarget(dialog!, actionL #selector(BaseDialog.didClickOkButton(button:)), for: .
			touchUpInside)
		dialog?.show()
	}
	...
	//被动触发
	public func showAddSafeZone(name: String,
								clickHandler: @escaping DialgClickHandler,
								clickNeverHandler: @escaping DialogClickHandler) {
		let type = DialogManager.DialogType.addSafeZone
		if currentDialog?.dialogType = type {
			//当前对话框已显示,直接return
			return
		}
		var dialog 		= dialogQueue.first(where: { $0.dialogType == type })
		...
		if dialog == nil {
			...
			view?.okButton.addTarget(dialog!, action: #selector(BaseDialog.didClickButton(button:)), for: .
				touchUpInside)
			view?.neverButton.addTarget(dialog!, action: #selector(BaseDialog.didClickNeverButton(button:)), for: .
				touchUpInside)
		}
	...
	public func showQueue(title: String? = nil,
						  ...
						  clickOkHandler: DialogClickHandler? = nil,
						  dismissHandler: DialogClickHandler? = nil) {
		let type = DialogMamager.DialogType.message
		if currentDialog?.dialogType == type {
			//当前对话框已经显示，直接return
			return
		}
		...
		if dialog == nil {
			...
			//上部
			dialog = BaseDialog.create(view: veiw)
			if image == nil {
				dialog?.hideImageView()
			}
	...
	public func add(type: DialogType,
		...
		add(type: type, mynts: mynts, selectedHandler: selectedHandler, dismissHandler: dismissHandler,
			neverHandler: nil)
	}
	...
	public func add(type: DialogType,
		...
		if dialog == nil {
			...
			dialog?.dialogClickDismissHandler = dismissHandler
			if neverHandler != nil {
				dialog?.dialogClickNeverHandler = { dialog in
					let mynts = view?.mynts
					neverHandler?(dialog, mynts == nil ? [] : mynts!)
				}
			}
		}
		//添加数据
		(dialog?.dialogView as? CollectionDialogView)?.dialogSelectedHandler = selectedHandler
		(dialog?.dialogView as? CollectionDialogView)?.addMynts(mynt: mynts)
	...
	public func remove(type: DialogType, mynt: Mynt) {
		...
		(dialogQueue.first(where: { $0.dialogType == type })?.dialogView as? CollectionDialogView)?.removeMynt(mynt:
			mynt)
	}
	...
	public enum DialogType: Int {
		...
		var image: UIImage? {
			...
			case .addSafeZone, .deleteSafeZone, .clearSafeZone:
				return UIImage(named: "dialog_safezone")
			case .askHelp:
				return UIImage(named: "ask_friends_for_help_popup")
			case .alarmTime:
				return UIImage(named: "dialog_forget")
			default:
				return nil
		...
		var title: String {
			...
			case .askHelp:
				return NSLocalizedString("SHARE_ASK_FRIEND". comment: "求助")
			case .addSafeZone, deleteSafeZone, .clearSafeZone:
				return NSLocalizedString("SECUTRE_AREA_SAFE_ZONE", comment: "安全区域")
			case .buy:
		...
		var message: String {
			...
			case .deleteSafeZone:
				return NSLocalizedString("SECURE_AREA_DELETE_CONNECT", comment: "安全区域")
			case .clearSafeZone:
				return NSLocalizedString("SECURE_AREA_CLEAN_CONTENT", comment: "安全区域")
			case .alarmTime:
				return NSLocalizedString("您的小觅设备未携带或者运动过少，请抽空查看一下吧!", comment: "定时报警")
			default:
				return ""
			}
		}
		...
		var buttonString: String {
			...
			case .logout:
				return NSLocalizedString("SIGN_OUT_COMFROM", comment: "注销")
			case .permission:
				return NSLocalizedString("ADD_OK", comment: "确定")
			case .buy:
