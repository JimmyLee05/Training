...
extension LOTAnimationView {
	...
	fileprivate static var logicAnimation 	= "logicAnimation"
	...
	fileprivate var isPlayBack: Bool {
		get {
			let value = objc_getAssociatedObject(self, &LOTAnimationView.isPlayBack) as? Bool
			return value == nil ? false : value!
		} set(newValue) {
			objc_setAssociatedObject(self, &LOTAnimationView.isPlayBack, newValue, objc_AssociationPolicy.
				OBJC_ASSOCIATION_RETAIN)
		}
	}
	...
	func playBack(start: CGFloat, end: CGFloat, isPlayBack: Bool = false) {
		...
		progressTime 				= Timer.scheduledTimer(timeInterval: TimeInterval(animationDuration / 100),
														   target: self,
														   selector: #selector(_playBack),
														   uaerInfo: nil,
														   repeats: true
	}
	...
	@objc private func _play() {
		self.animationProgress = (self.animationProgress + 0.01) >= endProgress ? endProgress : (self.
	...
	@objc private func _playBack() {
		...
		if isPlayBack {
			...
			self.animationProgress += 0.01 * CGFloat(logicAnimation)
		} else {
