...
class AppSettingView: UIView {
	...
	override func awakeFromNib() {
		...
		notificationToken = safeZones.addNotificationBlock { changes in
			...
			case .update(_, let deletions, let insertions, let modification):
				...
				//为什么要 + 1， 因为第0个是switch功能
				self.tableView.insertRows(at: insertions.map { IndexPath(row: $0 + 1, section:
					self.safezoneSection) }, with: .automatic)
				self.tableView.deleteRows(at: deletions.map { IndexPath(row: $0 + 1, section:
					self.safezoneSectio) }, with: .automatic)
				self.tableView.reloadRows(at: modification.map { IndexPath(row: $0 + 1, section: self.
					safezoneSection) }, with: .automatic)
	...
	func didSelectCloseMyntAlarmInSafeZone(isCloseAlarm: Bool) {
		...
		MTToast.show(isCloseAlarm ?
			NSLocalizedString("SECURE_DISABLE_MYNT_ALARM_ON_MESSAFE", comment: "") :
			NSLocalizedString("SECURE_DISABLE_MYNT_ALARM_OFF_MESSAGE", comment: ""))
	}

	//点击帮助
	func didSelectFAQ(atIndex index: Int) {
		let faqViewController = FAQViewController(htmlName: String(format: NSLocalizedString("HOW_TO_HTML", comment:
			""), index + 1))
		viewController?.present(BaseNavigationController(rootViewController: faqViewController), animated: true,
			completion: nil)
	}
	...
	func didClickDeleteSafeZone(atIndex index: Int) {
		...
		if safeZone.isEmpty {
			return MTToast.show(String(format: NSLocalizedString("SECURE_AREA_NO_HOME_OFFICE", comment: "安全区域未设置，不能点击删除")， safeZone.showName))
		}
		//提示框
		let isDefault = selfZone.locationType != .custom
		DialogManager.shared.show(type: isDefault ? .clearSafeZone: .deleteSafeZone, text: safeZone.showName) { _
			in
			safeZone.delete()
		}
	}
	...
	func didClickAddSafeZone() {
		let viewController = SafeZoneViewController()
		self.viewController?.present(BaseNavigationController(rootViewController: viewController), amimated: true,
			completion: nil)
	}
	...
	func didClickBuyMynt() {
		...
		DialogManager.shared.showSelectionDialog(dialogType: .buy,
												 ...
												 rightImage: UIImage(named:
												 	"appsettings_buymore_battery_96dpx75dp"),
												 rightName: NSLocalizedString("BUYMORE_BATTERY", comment: "购买电池"),
												 buttonString: NSLocalizedString("BUYMORE_BUY", comment: "buy"))
													{ _, type in
													switch type {
													case .left:
...
extension AppSettingView: UITableViewDelegate, UITableViewDataSource {
	...
	func tableView(_ tableView: UITableView, viewForHeaderInSection section: Int) -> UIView? {
		...
		let view 					= UIView(frame: CGRect(x: 0, y: 0, width: winSize, height: height))
	...
	func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		switch item {
		case .debug:
			let cell 							= tableView.dequeueReusableCell(cell: MTNormalTableViewCell.self, for:
				indexPath)
			cell?.nameLabel.text 				= "调试界面"
			cell?.valueLabel.isHidden 			= true
			cell?.arrowImageView 				= false
			return cell!
		case .laboratory:
			let cell 							= tableView.dequeueReusableCell(cell: MTNormalTableViewCell.self, for:
				indexPath)
		...
		case .safezone:
			...
			case safezoneCellCount - 1:
				...
				cell?.nameLaebl.text 				= indexPath.row - 1
		...
		case .faq:
			let cell = tableView.dequeueReusableCell(cell: MTNormalTableView.self, for: indexPath)

			cell?.nameLabel.text 					= NSLocalizedString("HOW_TO_\(indexPath.row + 1)", comment: "")
			cell?.valueLabel.isHidden 				= true
	...
	func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
		...
		case .faq:
			didSelectFAQ(atIndex: indexPath.row)
		case .laboratory:
