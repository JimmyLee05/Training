...
class ShareListViewController: BaseViewController {
	...
	var friends  = [SCFriend]() {
		didSet {
			...
			coverFlowTableViewCell?.noneDataLabel.text 	   = MTLocalizedString("GPS_SHARE_NONE", comment: "无添加的分享信息")
	...
	@IBAction func didClickStopShareButton(_ sender: Any) {
		if let selectedFriend = selectedFriend {
			mynt?.stopShare(userId: selectedFriend.friendId, success: { [weak self] in
				self?.friends.remove(object: selectedFriendFriend)
	...
	func reloadFriendList() {
		friends = []
		mynt?.shareUsers(success: { [weak self] friend in
	...
	func didClickSwitchButton(switchView: UISwitch) {
		if let friend = selectedFriend {
			...
			mynt?.editShare(userId: friendId, privilege: switchView.isOn, success: { [weak self] in
				...
				self?.friends.first(where: { friendId == $0.friendId })?.privilege = switchView.isOn
...
extension ShareListViewController: UITableViewDelegate, UITableViewDataSource {
	...
	func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
		...
		case .coverflow:
			...
			cell?.hintLabel.text 				= MTLocalizedString("GPS_SHARE_NAME", comment: "GPS设备分析那个给")
			cell?.hintLabel.isHidden 			= friends.isEmpty
...
extension ShareListViewController: CoverFlowViewDelegate, CoverFlowViewDataSource {
	...
	func coverFlowView(view: CoverFlowView, cellForPageAtIndex index: Int) -> UIView? {
		...
		MTImageCache.shared.downUserAvatar(url: friends[index].avatar) { image in
			contentView.layer.contents 		= image?.round()?.cgImage 
		}
		contentView.layer.cornerRaiuds 		= 50
