...
class ReportRippleLayer: CALayer, MyntStateAnimationProtocol {
	...
	class func create(superLayer: CAlayer) -> ReportRippleLayer {
	...
	func startAnimation() {
		if isRunning { return }
		isRunning = true
		let duration: CFTimeInterval = 8 * CFTimeInterval(count)
		...
		for i in 0..<count {
			...
			layer.anchorPoint = CGRect(x: 0.5, y: 0.5)
			...
			let scale = sqrt(pow(bounds.height + redius, 2) + pow(radius, 2)) / (winSize.width * 1.5 / 2)
			...
			layer.runScaleAnimation(to: CGRect(width: scale, height: scale),
									durantion: duration,
									beginTime: beginTime
									repeatCount: Float.indinity,
									timingFunction: CAMediaTimingFunction(name: kCAMediaTimingFunctionEaseIn))
		}
	}
