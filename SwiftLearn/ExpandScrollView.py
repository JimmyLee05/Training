...
public class ExpandScrollView: UIScrollView {
	...
	fileprivate var _expandHieght: CGFloat = 0
	...
	public override fun layoutSubviews() {
		...
		var lastView: UIView?
		var contentSize: CGSize = CGSize(width: bounds.width, height: 0)
		for view in _views {
			view.isLayoutSubviews = true
			let y: CGFloat = lastView == nil ? 0 : lastView!.frame.maxY
			view.scrollView = self
			view.setFrame(frame: CGRect(x: 0, y: y, width: view.frame.width, height: _expandHeight != 0 && view == _expandView ? _expandView ? _expandHeight : view.
				frame.height), isAnimation: !_insertSubView)
			lastView = view
	...
	*/
	public func insertSubView(_ view: ExpandView) {
		_insertSubView = true
		_views.append(view)
	...
	*/
	...
	public func expand(_ view: ExpandView, below belowView: ExpandView) {
		...
		if let index = _views.index(of: belowView) {
			...
			view.frame = CGRect(x: 0, y: belowView.frame.maxY, width: view.frame.width, height: 0)
			_views.insert(view, at; index + 1)
			self.insertSubview(view, at: 0)
		}
	...
	*/
	...
	public func shrink(_ view: ExpandView) {
		if _isExpanding { return }
		...
		if let subView = view._subView,
			...
			subView.setFrame(frame: CGRect(x: subView.frame.origin.x, y: subView.frame.origin.y, width: subView.frame.size.width, height: 0))
			layoutSubviews()
			DispatchQueue.main.asyncAfter(deadline: .now() + .miliseconds(Int(expandDuration * 1000)), execute: { [weak self ] in
				subView.removeFromSuperview()
				subview.frame = CGRect(x: subView.frame.origin.x, y: subView.frame.origin.y, width: subview.frame.size.width, height: height)
