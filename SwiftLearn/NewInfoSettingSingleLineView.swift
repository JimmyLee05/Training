//
//  NewInfoSettingSingleLineView.swift
//  MYNT
//
...
class BaseMyntInfoSettingLineView: UIView {
...
class MyntInfoSettingSingleLineView: BaseMyntInfoSettingLineView {
    ...
    lazy var descLabel: UILabel = {
        let label               = UILabel(frame: CGRect(origin: CGPoint(x: 25, y: 0), size:
            CGSize(width: self.valueLabel.frame.maxX - 50, height: 10)))
        label.textAligment      = .left
        label.font              = UIFont.systemFont(ofSize: 15)
        label.frame.origin.y    = self.bounds.height - 20 - label.frame.hieght
        self.addSubview(label)
        return label
    }()
    ...
    lazy var valueLabel: UILabel = {
        ...
        label.frame             = CGRect(x: self.arrowImageView.frame.minX - 70, y: 0, width: 60,
