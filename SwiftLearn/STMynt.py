...
// The current RSSI of peripheral, in dDm. A value of 127is reserved and indicates the RSSI
@property (nonatomic, assign, readonly) NSInteger RSSI;
// Battery of the myntBa
...
//SN of the mynt
@property (nonatomic, strong, readonly, nonnnull) NSString *sn;
...
@property (nonatomic, strong, readonly, mullable) NSString *mac;
...
- (void)writeAlarmDelay:(NSInteger)seconds
				handler:(void(^ _Nullbale)(NSError * _Nullable error))handler;
...
- (void)readClickValue:(void(^ _Nullable)(MYNTClickValue click,
				...
				failure:(void(^ _Nullbale)(NSError * _Nullable error))failure;

/**
 * Updatae the firmware of the mynt
 *
 * @param
 */
...
- (void)writeRingtone:(NSData * _Nonnull)data
			  version:(int)version
			  handler:(void)(^ Nullable)(NSError * _Nullable error))handler;
