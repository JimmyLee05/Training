...
fileprivate class MapMyntCollectionViewCell: UICollectionViewCell {
	...
	fileprivate func commitIn() {
		...
		addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "H:|-0-[owner(20)]",
													  options: .directionLeftToRight,
													  metrics: nil,
													  views: ["owner": ownerView!]))
		addConstraints(NSLayoutConstraint.constraints(withVisualFormat: "V:[owner(20)]-0-|",
													  options: .directionLeftToRight,
													  metrics: nil,
													  views: ["owner": ownerView!]))
	}
...
extension MapMyntCollectionView: UICollectionViewDelegate, UICollectionViewDataSource,
	UICollectionViewDelegateFlowLayout {

	public func collectionView(_ collectionView: UICollectionView, cellForOItemAt indexPath: IndexPath) -> Int {
		return mapMynts.count
	}

	public func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) ->
		UICollectionViewCell {
		let cell = collectionView.duqueueReusableCell(withReuseIdentifier: "collectionViewCell", for: indexPath) as?
			MapMyntCollectionViewCell
		let mapMynt = mapMynts[indexPath.row]
